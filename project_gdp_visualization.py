...
def from_key_to_value(key):
    for k,v in pygal_countries.items():
        if k == key:
           ...
           #通过定义value_to_key,key_to_value两函数实现国家代码缩写与国名的自由转换
...
love = read_csv_as_nested_dict("isp_gdp.csv","Country Name",",",'"')      #运行函数
# print(love) #查看
alpha = reconcile_countries_by_name(pygal_countries,love)                 #运行第二个函数，并将结果存为变量alpha
beta = build_map_dict_by_name(love,alpha,year)                             #运行第三个函数，并将结果存为变量beta，便于引用
